...
def scale(kernel):
    ...
    return [[int(255*(k-minimum)/(maximum-minimum)) for k in row] for row in kernel]
...
def generate_gaussian(sigma=3.0, size=21):
    ...
    gauss = lambda x, y: 1/(2*pi*sigma) * exp(-(x**2 + y**2)/ (2 * sigma**2))
    return [[gauss(x-(size-1)/2, y-(size-1)/2)
           for y in range(size)] for x in range(size)]

def generate_fourier(wavelength=8, direction=0, size=21):
    """ Generates a 2D Fourier function scaled to [0, 255] """
    fourier = lambda x, y: cos(2*pi/wavelength * (x*cos(direction) + y*sin(direction)))
    return [[fourier(x-(size-1)/2, y-(size-1)/2)
           for y in range(size)] for x in range(size)]
...
def generate_gabor(wavelength=8, direction=0, sigma=None, size=21):
    ...
    if sigma == None:
        octave = 1
        sigma = wavelength * 1/pi * sqrt(log(2)/2) * (2**octave+1)/(2**octave-1)
